...
const Addrole = () => {
    ...
    const success = () => toast.success("Role Added Successfully!");
    ...
    let send_data = async (values) =>{
        try{
            ...
            let responce = await axios.post('http://localhost:4000/role/role',values, {headers});
            handleReset();
            if(responce.data.status == "role is already Exist")
            {
    ...
    let check = Yup.object({
        name:Yup.string().min(2, "Minimum 2 character Required").max(18,"It's To Long For Role").required("Role Must Be Required"),
    });
        ...
        <>
            <div className="add-role">
                <div className="d-flex justify-content-between align-items-center my-3 px-3">
                    ...
                    <span className='home'><Link to={'/dashboard'}>Home</Link> / Add Role</span>
                </div>
                <div className="roll-form">
                    <Row className='justify-content-center mt-5'>
                        <Col lg={4} className='rounded-4 shadow form'>
                            <div className="addroll-form mt-2">
                                <h3 className='form-heading text-center'>Add Role</h3>
                            </div>
                            <div className='mt-3'>
                                <form method='post' className='m-auto px-4' onSubmit={handleSubmit}>
                                    <table className='w-100'>
                                        ...
                                        <tr>
                                            <td className='text-center'><input type="submit" value="Add Role" className='my-2 px-4 py-2 rounded submit'/></td>
                                        </tr>
                                    </table>
                                </form>
                            </div>
